...
(function () {
  ...
  function getCmConfig() {
    ...
    if ($tw.utils.isArray(configTiddlers)) {
      for (var i = 0; i < configTiddlers.length; i++) {
        ...
        if (tiddler) {
          element = configTiddlers[i].replace(
            /\$:\/config\/codemirror\//gi,
            '',
          );
          type = tiddler.fields.type
            ? tiddler.fields.type.trim().toLocaleLowerCase()
            : 'string';
          switch (type) {
            case 'bool':
  ...
  function CodeMirrorEngine(options) {
    ...
    config.placeholder = $tw.wiki.getTiddlerText(
    ...
    this.cm.on('drop', function (cm, event) {
      ...
      if (
        ...
        function posFromMouse(cm, e, liberal, forRect) {
          ...
          let coords = cm.coordsChar(cm, x, y),
            line;
          if (
            forRect &&
            coords.xRel > 0 &&
            (line = cm.getLine(cm.doc, coords.line).text).length == coords.ch
          ) {
            let colDiff =
              window.CodeMirror.countColumn(
                line,
                line.length,
                cm.options.tabSize,
              ) - line.length;
            coords = window.CodeMirror.Pos(
              coords.line,
              Math.max(
                0,
                Math.round(
        ...
        try {
          ...
          if (text) {
            ...
            cm.setCursor(
              cm.coordsChar({ left: event.pageX, top: event.pageY }),
            );
            if (selected) {
    ...
    this.cm.on('keydown', function (cm, event) {
      if (
        $tw.keyboardManager.handleKeydownEvent(event, { onlyPriority: true })
      ) {
        return true;
      }
...
*/
  CodeMirrorEngine.prototype.fixHeight = function () {
    if (this.widget.editAutoHeight) {
      ...
      var fixedHeight = parseInt(
        this.widget.wiki.getTiddlerText(HEIGHT_VALUE_TITLE, '400px'),
        10,
      );
...
*/
  CodeMirrorEngine.prototype.createTextOperation = function () {
    ...
    operation.selection = operation.text.substring(
      operation.selStart,
      operation.selEnd,
    );
    return operation;
  ...
  CodeMirrorEngine.prototype.executeTextOperation = function (operation) {
    ...
    if (operation.replacement !== null) {
      this.cm.replaceRange(
        ...
        this.cm.posFromIndex(operation.cutStart),
        this.cm.posFromIndex(operation.cutEnd),
      );
      this.cm.setSelection(
        this.cm.posFromIndex(operation.newSelStart),
        this.cm.posFromIndex(operation.newSelEnd),
      );
      newText =
        operation.text.substring(0, operation.cutStart) +
        operation.replacement +
