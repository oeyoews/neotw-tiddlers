...
另一方面，如果 [TLS](https://developer.mozilla.org/zh-CN/docs/Glossary/TLS) 桥接代理通过在 PC 上安装来自组织管理的 [CA](https://developer.mozilla.org/zh-CN/docs/Glossary/Certificate_authority) 证书，以中间人方式解密所有通信，并执行访问控制等，则可以查看响应的内容并将其缓存。但是，由于[证书透明度（certificate transparency）](https://developer.mozilla.org/zh-CN/docs/Web/Security/Certificate_Transparency)在最近几年变得很普遍，并且一些浏览器只允许使用证书签署时间戳（signed certificate timestamp）颁发的证书，因此这种方法需要应用于企业策略。在这样的受控环境中，无需担心代理缓存 “已过时且未更新”。
...
例如，Varnish Cache 使用 VCL（Varnish Configuration Language，一种 [DSL](https://developer.mozilla.org/zh-CN/docs/Glossary/DSL/Domain_specific_language)）逻辑来处理缓存存储，而 service worker 结合缓存 API 允许你在 JavaScript 中创建该逻辑。
...
![缓存的类型](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Caching/type-of-cache.png)
...
## [基于 age 的缓存策略](#基于_age_的缓存策略)

存储的 HTTP 响应有两种状态：**fresh** 和 **stale**。*fresh* 状态通常表示响应仍然有效，可以重复使用，而 *stale* 状态表示缓存的响应已经过期。

确定响应何时是 fresh 的和何时是 stale 的标准是 **age**。在 HTTP 中，age 是自响应生成以来经过的时间。这类似于其他缓存机制中的 [TTL](https://developer.mozilla.org/en-US/docs/Glossary/TTL "此页面目前仅提供英文版本")。
...
* 如果响应的 age *小于*一周，则响应为 *fresh*。
* 如果响应的 age *超过*一周，则响应为 *stale*。
...
## [Expires 或 max-age](#expires_或_max-age)
...
![使用 url 作为键](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Caching/keyed-with-url.png)
...
![使用 url 和语言作为键](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Caching/keyed-with-url-and-language.png)
